...
> "0 问题，全部通过。"——AI 每次自信满满说出这句话的时候，我都会想起它三分钟后自己打脸的样子。

你大概也遇到过：AI 批量改完几十处，回头告诉你"我检查过了，没问题"。你信了，然后下一轮认真复查，翻出一堆漏网之鱼。不是它存心骗你——是它真的只用了一路快速检查就下了结论。这个现象叫**假收敛**，在 AI 改长文档、批量修复、改 skill 自身之后尤其高发。

本 skill 把「真循环铁律」固化成协议：**审查 → 修复 → 重新审查**，直到问题数单调递减到 0，且连续 N 轮重新审查都无新问题，才算真的收敛——并内置 4 层过拟合防护，防止"越修越多、越改越糊"。
